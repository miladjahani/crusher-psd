// src/screens/ReportsScreen.tsx — PSD dashboard: KPIs + chart + table + export + history.
import { BarChart3, FileSpreadsheet, FileText, FlaskConical, FolderOpen, PenLine, ScanLine, Trash2 } from 'lucide-react';
import { greg, jalali, useI18n, type TKey } from '../i18n';
...
export function ReportsScreen({ record, history, accent, onLoad, onDelete, onNavigate, buzz, notify }: {
  record: AnalysisRecord | null; history: AnalysisRecord[]; accent: string;
  onLoad: (r: AnalysisRecord) => void; onDelete: (id: string) => void; onNavigate: (t: Tab) => void; buzz: Buzz; notify: Notify;
}) {
    ...
    return (
      <div className="fade-up space-y-4">
        ...
        <div className="flex flex-col items-center gap-4 rounded-3xl border-2 border-dashed border-white/10 bg-white/[0.02] px-6 py-14 text-center">
          ...
          <div><div className="text-base font-black">{t('noData')}</div><p className="mx-auto mt-1 max-w-xs text-xs leading-relaxed text-zinc-500">{t('noDataHint')}</p></div>
  ...
  const L: ReportStrings = { psd: t('psdChart'), sample: t('sampleName'), date: t('date'), sourceLabel: t('sourceLabel'), sieve: t('sieveSize'), weight: t('colRetained'), retPct: t('colRetPct'), cumRet: t('colCumRet'), passing: t('colPassing'), eff: t('effectiveSize'), med: t('medianSize'), coarse: t('coarseSize'), grad: t('gradation'), gradVal: t(gradKey) };
  ...
  return (
    <div className="fade-up space-y-4">
      ...
      <Card className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate text-[11px] text-zinc-500">{record.source === 'ai' ? t('sourceAI') : t('sourceManual')} • {num(jalali(record.createdAt))} • {greg(record.createdAt)}</div>
          </div>
        </div>
      ...
      <Card>
        <div className="flex items-center justify-between"><h3 className="text-sm font-black">{t('psdChart')}</h3><span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">{t('percentPassing')}</span></div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => { buzz(15); if (!exportPDF(r, meta, L)) notify('Popup blocked — allow popups.'); }} className="flex min-h-[60px] flex-col items-center justify-center gap-1 rounded-2xl border-2 border-[#ffc400]/70 bg-[#ffc400]/10 px-2 text-center text-[11px] font-black leading-tight text-[#ffc400] transition active:scale-[0.97]"><FileText size={20} /> {t('downloadPDF')}</button>
          <button onClick={() => { buzz(15); exportExcel(r, meta, L); notify(`${t('exportExcel')} ✓`); }} className="flex min-h-[60px] flex-col items-center justify-center gap-1 rounded-2xl bg-accent px-2 text-center text-[11px] font-black leading-tight text-ink-950 shadow-glow transition active:scale-[0.97]"><FileSpreadsheet size={20} /> {t('exportExcel')}</button>
        </div>
      </div>
      {history.length > 0 && (
        <div>
          <h3 className="mb-2 px-1 text-sm font-black">{t('history')} <span className="text-zinc-500">({num(history.length)})</span></h3>
            ...
            {history.map((h) => (
              <div key={h.id} className={`flex items-center gap-3 rounded-2xl border p-3 transition ${h.id === record.id ? 'border-accent/50 bg-accent/5' : 'border-white/10 bg-white/[0.03]'}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[11px] text-zinc-500">{h.source === 'ai' ? t('sourceAI') : t('sourceManual')} • {num(jalali(h.createdAt))} • D50 {fmtSize(h.result.d50)} mm</div>
                </div>
                <button onClick={() => onLoad(h)} aria-label={t('load')} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 text-zinc-300 transition hover:border-accent/50 hover:text-accent active:scale-90"><FolderOpen size={16} /></button>
                <button onClick={() => onDelete(h.id)} aria-label={t('delete')} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 text-zinc-500 transition hover:border-red-400/50 hover:text-red-400 active:scale-90"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
        </div>
