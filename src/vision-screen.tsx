// src/vision-screen.tsx — wires vision.ts + edges.ts + db.ts into the Visual tab.
// Real Canny/Sobel views + robust segmentation + 3 calibration modes + IndexedDB save.
// Local EN/FA strings (keeps i18n.tsx untouched -> safe build). RTL via document dir.
...
import {
  analyze, imageDataFromImage, makeCalibrationLine, makeCalibrationCircle, makeCalibrationNumeric,
  maskToImageData, labelsToImageData, buildSieveFromDiameters,
  type VisionResult, type Calibration,
} from './vision';
...
const L = {
  en: {
    title: 'Optical Granulometry', desc: 'Real Canny/Sobel edges + robust segmentation. Calibrate with a line, a coin/ball, or a number.',
    upload: 'Upload sample image', hint: 'Spread particles on a flat surface for best accuracy',
    raw: 'Raw', edges: 'Edges', mask: 'Mask', labels: 'Segments', boxes: 'Bounding boxes',
    canny: 'Canny', sobel: 'Sobel',
    cal: 'Scale calibration', line: 'Line', circle: 'Coin / Ball', numeric: 'Numeric',
    lineHint: 'Drag a line over a known length', circleHint: 'Drag a circle over the coin/ball', numericHint: 'Image width equals (mm)',
    realLen: 'Real length (mm)', diameter: 'Diameter (mm)', frameMm: 'Frame width (mm)',
    blur: 'Blur', low: 'Low thr', high: 'High thr', invert: 'Invert', fast: 'Fast', accurate: 'Accurate',
    count: 'Particles', mean: 'Mean ⌀', d50: 'D50', conf: 'Confidence',
    high: 'High', medium: 'Medium', lowC: 'Low',
    notCal: 'Not calibrated — sizes shown in pixels. Set a scale to get mm.',
    heap: 'Low confidence: likely a packed heap. Spread a single layer for reliable numbers.',
    table: 'Particle table', id: '#', area: 'Area(px)', ar: 'AR', deq: 'deq',
    sample: 'Sample name', save: 'Save scan', saved: 'Saved ✓', send: 'Send to report',
    needCal: 'Calibrate first (need mm) to build a PSD curve.', needMore: 'Need ≥3 measured particles.',
    clear: 'Clear', px: 'px',
  },
  fa: {
    title: 'دانه‌بندی اپتیکی', desc: 'لبه‌یابی واقعی Canny/Sobel + بخش‌بندی مقاوم. با خط، سکه/توپ یا عدد کالیبره کنید.',
    upload: 'بارگذاری تصویر نمونه', hint: 'برای دقت بهتر ذرات را روی سطح صاف پخش کنید',
    raw: 'خام', edges: 'لبه', mask: 'ماسک', labels: 'بخش‌ها', boxes: 'کادر محیطی',
    canny: 'Canny', sobel: 'Sobel',
    cal: 'کالیبراسیون مقیاس', line: 'خط', circle: 'سکه / توپ', numeric: 'عددی',
    lineHint: 'روی یک طول معلوم خط بکشید', circleHint: 'دور سکه/توپ دایره بکشید', numericHint: 'عرض تصویر برابر (mm)',
    realLen: 'طول واقعی (mm)', diameter: 'قطر (mm)', frameMm: 'عرض کادر (mm)',
    blur: 'محو', low: 'آستانه پایین', high: 'آستانه بالا', invert: 'معکوس', fast: 'سریع', accurate: 'دقیق',
    count: 'ذرات', mean: 'میانگین ⌀', d50: 'D50', conf: 'اعتماد',
    high: 'بالا', medium: 'متوسط', lowC: 'پایین',
    notCal: 'کالیبره نشده — اندازه‌ها بر حسب پیکسل‌اند. برای mm مقیاس بدهید.',
    heap: 'اعتماد پایین: احتمالاً کپه‌ی فشرده. یک لایه پخش کنید.',
    table: 'جدول ذرات', id: '#', area: 'مساحت(px)', ar: 'AR', deq: 'deq',
    sample: 'نام نمونه', save: 'ذخیره اسکن', saved: 'ذخیره شد ✓', send: 'ارسال به گزارش',
    needCal: 'اول کالیبره کنید (mm لازم است) تا منحنی PSD ساخته شود.', needMore: 'حداقل ۳ ذره‌ی اندازه‌گیری‌شده لازم است.',
    clear: 'پاک کردن', px: 'px',
  },
} as const;
...
export default function VisionScreen({ showGrid, onResult, buzz, notify }: Props) {
  ...
  const tr = (k: keyof typeof L.en) => L[lang][k];
  ...
  function runEngine() {
    if (!imgEl) return;
    try {
      const data = imageDataFromImage(imgEl, 700);
  ...
  useEffect(() => { draw(); }, [res, edge, baseView, showBoxes, calLine, calCircle, showGrid, accent]);
  ...
  function draw() {
    ...
    if (showGrid) {
      ctx.strokeStyle = `rgb(${accent} / 0.18)`; ctx.lineWidth = 1;
    ...
    if (showBoxes) {
      ctx.strokeStyle = accentCss; ctx.fillStyle = accentCss; ctx.lineWidth = 1; ctx.font = '9px ui-monospace, monospace';
      const list = res.particles.slice(0, 500);
      for (const p of list) {
        ctx.strokeRect(p.bbox.x + 0.5, p.bbox.y + 0.5, p.bbox.w, p.bbox.h);
        if (list.length <= 120) ctx.fillText(p.deqMm != null ? p.deqMm.toFixed(1) : p.deqPx.toFixed(0), p.bbox.x + 1, p.bbox.y + 9);
      }
    }
    ctx.strokeStyle = '#ffc400'; ctx.fillStyle = '#ffc400'; ctx.lineWidth = 2;
    if (calMode === 'line' && calLine) {
      ctx.beginPath(); ctx.moveTo(calLine[0].x, calLine[0].y); ctx.lineTo(calLine[1].x, calLine[1].y); ctx.stroke();
      for (const p of calLine) { ctx.beginPath(); ctx.arc(p.x, p.y, 3, 0, 7); ctx.fill(); }
    }
    if (calMode === 'circle' && calCircle) {
      ctx.beginPath(); ctx.arc(calCircle.c.x, calCircle.c.y, calCircle.r, 0, 7); ctx.stroke();
      ctx.beginPath(); ctx.arc(calCircle.c.x, calCircle.c.y, 3, 0, 7); ctx.fill();
    }
  }

  function getPos(e: RPointer<HTMLCanvasElement>): Pt {
    const cv = canvasRef.current!; const r = cv.getBoundingClientRect();
    return { x: (e.clientX - r.left) * (cv.width / r.width), y: (e.clientY - r.top) * (cv.height / r.height) };
  }
  function onDown(e: RPointer<HTMLCanvasElement>) {
    if (calMode === 'numeric') return;
    (e.target as Element).setPointerCapture?.(e.pointerId);
    const p = getPos(e); setDrawing(true);
    if (calMode === 'line') setCalLine([p, p]); else setCalCircle({ c: p, r: 0 });
  }
  function onMove(e: RPointer<HTMLCanvasElement>) {
    if (!drawing) return; const p = getPos(e);
    if (calMode === 'line') setCalLine((c) => (c ? [c[0], p] : null));
    else setCalCircle((c) => (c ? { c: c.c, r: dist(c.c, p) } : null));
  }
  ...
  function onFile(f?: File) {
    ...
    im.onload = () => { setImgEl(im); setCalLine(null); setCalCircle(null); buzz(12); };
    im.src = url;
  }
  ...
  function send() {
    ...
    const sieve = buildSieveFromDiameters(diams);
    const psd = computePSD(sieve);
    if (!psd) return notify(tr('needMore'));
    buzz(20);
          ...
          {s && (
            <>
              <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-[11px] font-bold ${s.confidenceLabel === 'low' ? 'border-amber-400/40 bg-amber-400/10 text-amber-300' : s.confidenceLabel === 'medium' ? 'border-white/15 bg-white/5 text-zinc-300' : 'border-accent/40 bg-accent/10 text-accent'}`}>
                <AlertTriangle size={14} />{tr('conf')}: {s.confidenceLabel === 'high' ? tr('high') : s.confidenceLabel === 'medium' ? tr('medium') : tr('lowC')} — {s.confidenceReasons.join(', ')}
              </div>
...
function Slider({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (v: number) => void }) {
  return (
    <label className="block">
      <span className="flex justify-between text-zinc-400"><span>{label}</span><span dir="ltr" className="text-zinc-300">{value || 'auto'}</span></span>
      <input type="range" min={min} max={max} value={value} onChange={(e) => onChange(parseInt(e.target.value, 10))} className="mt-1 w-full" />
    </label>
  );
}
...
function Kpi({ label, value, unit, color }: { label: string; value: string; unit?: string; color?: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
      ...
      <div className="mt-1 text-lg font-black tabular-nums" style={color ? { color } : undefined}>{value}{unit && <span className="ms-1 text-[10px] text-zinc-500">{unit}</span>}</div>
    </div>
  );
}
