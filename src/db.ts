// src/db.ts (v2.1) — session-based IndexedDB + analytics helpers + legacy migration + saveScan wrapper.
...
export interface Session { id: string; name: string; location?: string; operator?: string; color?: string; createdAt: number; updatedAt: number; notes?: string; }
...
async function ensureMigrated(db: IDBDatabase): Promise<void> {
  ...
  for (const sc of scans) {
    ...
    const res: Result = { id: sc.id, sessionId: QUICK_ID, kind, sampleName: sc.sampleName || 'Imported', createdAt: sc.createdAt || Date.now(), sieve: sc.sieve || [], d10: null, d50: sc.stats?.d50MassMm ?? null, d80: null, cu: null, cc: null, gradation: 'gap', calibration: sc.calibration, stats: sc.stats, particles: sc.particles, thumbBlob: sc.thumbBlob || null, hasImage: !!sc.hasImage };
    store.put(res);
  }
...
async function ensureQuick(db: IDBDatabase): Promise<void> {
  ...
  if (!q) { const now = Date.now(); const quick: Session = { id: QUICK_ID, name: 'Quick', createdAt: now, updatedAt: now, notes: 'Default session for quick captures' }; const tx = db.transaction(S_SESSIONS, 'readwrite'); tx.objectStore(S_SESSIONS).put(quick); await txDone(tx).catch(() => {}); }
}
let readyP: Promise<void> | null = null;
export function ensureReady(): Promise<void> { if (readyP) return readyP; readyP = (async () => { const db = await openDB(); await ensureMigrated(db); await ensureQuick(db); })(); return readyP; }
...
export async function createSession(p: { id?: string; name: string; location?: string; operator?: string; color?: string; notes?: string }): Promise<string> {
  await ensureReady(); const db = await openDB(); const now = Date.now(); const id = p.id || uid();
  ...
  const tx = db.transaction(S_SESSIONS, 'readwrite'); tx.objectStore(S_SESSIONS).put(s); await txDone(tx); return id;
}
export async function listSessions(): Promise<Session[]> { await ensureReady(); const db = await openDB(); const all = await req<Session[]>(db.transaction(S_SESSIONS, 'readonly').objectStore(S_SESSIONS).getAll()); return all.sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0)); }
export async function getSession(id: string): Promise<Session | null> { await ensureReady(); const db = await openDB(); const s = await req<Session | undefined>(db.transaction(S_SESSIONS, 'readonly').objectStore(S_SESSIONS).get(id)); return s || null; }
export async function updateSession(id: string, patch: Partial<Omit<Session, 'id' | 'createdAt'>>): Promise<void> {
  await ensureReady(); const db = await openDB(); const cur = await req<Session | undefined>(db.transaction(S_SESSIONS, 'readonly').objectStore(S_SESSIONS).get(id)); if (!cur) return;
...
export async function deleteSession(id: string): Promise<void> {
  ...
  await ensureReady(); const db = await openDB(); const tx = db.transaction([S_SESSIONS, S_RESULTS, S_IMAGES], 'readwrite');
  const all = await req<Result[]>(tx.objectStore(S_RESULTS).getAll()); const rstore = tx.objectStore(S_RESULTS), istore = tx.objectStore(S_IMAGES);
  for (const r of all) if (r.sessionId === id) { rstore.delete(r.id); istore.delete(r.id); }
  tx.objectStore(S_SESSIONS).delete(id); await txDone(tx);
}

/* results */
export async function saveResult(r: Result, imageBlob: Blob | null): Promise<string> {
  await ensureReady(); const db = await openDB(); const full: Result = { ...r, hasImage: !!imageBlob };
  ...
  await txDone(tx); return r.id;
}
export async function listResults(filter: ResultFilter = {}): Promise<Result[]> {
  await ensureReady(); const db = await openDB(); let all = await req<Result[]>(db.transaction(S_RESULTS, 'readonly').objectStore(S_RESULTS).getAll());
...
export async function getResult(id: string): Promise<{ record: Result; imageBlob: Blob | null } | null> {
  await ensureReady(); const db = await openDB(); const tx = db.transaction([S_RESULTS, S_IMAGES], 'readonly');
  const record = await req<Result | undefined>(tx.objectStore(S_RESULTS).get(id)); if (!record) return null;
...
export async function deleteResult(id: string): Promise<void> { await ensureReady(); const db = await openDB(); const tx = db.transaction([S_RESULTS, S_IMAGES], 'readwrite'); tx.objectStore(S_RESULTS).delete(id); tx.objectStore(S_IMAGES).delete(id); await txDone(tx); }
export async function clearResultsForSession(sessionId: string): Promise<void> {
  await ensureReady(); const db = await openDB(); const tx = db.transaction([S_RESULTS, S_IMAGES], 'readwrite');
  const all = await req<Result[]>(tx.objectStore(S_RESULTS).getAll()); const rstore = tx.objectStore(S_RESULTS), istore = tx.objectStore(S_IMAGES);
  for (const r of all) if (r.sessionId === sessionId) { rstore.delete(r.id); istore.delete(r.id); }
  await txDone(tx);
}
export async function countBySession(): Promise<Record<string, number>> { const all = await listResults(); const m: Record<string, number> = {}; for (const r of all) m[r.sessionId] = (m[r.sessionId] || 0) + 1; return m; }
export async function listLocations(): Promise<string[]> { const all = await listResults(); const s = new Set<string>(); for (const r of all) if (r.location) s.add(r.location); return [...s].sort(); }

/* bridge Result -> AnalysisRecord (recompute PSD points from sieve for charts/tables) */
export function resultToAnalysisRecord(r: Result): AnalysisRecord | null {
  const psd = computePSD(r.sieve || []); if (!psd) return null;
  return { id: r.id, sampleName: r.sampleName, source: r.kind === 'manual' ? 'manual' : 'ai', createdAt: r.createdAt, rows: r.sieve || [], result: psd };
}

/* legacy wrapper (keeps vision-screen.tsx working; writes into active session) */
...
export async function saveScan(rec: NewScan, imageBlob: Blob | null): Promise<string> {
  const id = rec.id || uid(); const createdAt = rec.createdAt || Date.now(); const sessionId = getActiveSessionId() || QUICK_ID;
  const result: Result = { id, sessionId, kind: rec.source, sampleName: rec.sampleName, createdAt, sieve: rec.sieve || [], d10: null, d50: rec.stats?.d50MassMm ?? null, d80: null, cu: null, cc: null, gradation: 'gap', calibration: rec.calibration, stats: rec.stats, particles: rec.particles, thumbBlob: rec.thumbBlob || null, notes: rec.notes };
